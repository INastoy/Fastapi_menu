...
class TestSubmenu:
    ...
    def setup_class(self):
        ...
        self.submenus_count = self.session.query(Submenu).filter(Submenu.menu_id == self.menu1.id).count()

    def test_get_submenus_ok(self, client, submenu):
        response = client.get(f'/api/v1/menus/{self.menu1.id}/submenus')
        submenus_from_db = [SubmenuSchema.from_orm(submenu) for submenu in submenu.get_all(menu_id=self.menu1.id)]
    ...
    def test_get_submenu_ok(self, client, submenu, session):
        ...
        submenu_from_db = SubmenuSchema.from_orm(submenu.get_by_id(submenu_id=self.submenu1.id, menu_id=self.menu1.id))
    ...
    def test_update_submenu_ok(self, client, submenu):
        ...
        assert submenu_from_db.description == self.data_for_update.get('description')
        assert response.json() == submenu_from_db

    def test_delete_submenu_ok(self, client, submenu):
        response = client.delete(f'/api/v1/menus/{self.menu1.id}/submenus/{self.submenu1.id}')
        assert response.status_code == 200
        with pytest.raises(HTTPException) as ex:
            submenu.get_by_id(submenu_id=self.submenu1.id, menu_id=self.menu1.id)
