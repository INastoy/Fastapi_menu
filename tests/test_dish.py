...
class TestDish:
    ...
    async def setup_class(self, session: AsyncSession):
        ...
        self.menu1 = await self.session.scalar(
            select(Menu).filter_by(title='dish_test_title1', description='dish_test_desc1'))
        ...
        await self.session.execute(insert(Dish)
                                   .values(
            title='test_title1',
        ...
        self.dishes_count = await self.session.scalar(
            select(func.count())
            .select_from(Dish)
            .filter_by(submenu_id=self.submenu1.id)
        )
    ...
    async def test_get_dishes_ok(self, client: AsyncClient, dish: DishCRUD):
        response = await client.get(
            f'/api/v1/menus/{self.menu1.id}/submenus/{self.submenu1.id}/dishes', follow_redirects=True
        )
        dishes_from_db = [
            DishSchema.from_orm(dish_data)
            for dish_data in await dish.get_all(submenu_id=self.submenu1.id)
        ]
    ...
    async def test_create_dish_ok(self, client: AsyncClient, dish: DishCRUD):
        ...
        dish_from_db = DishSchema.from_orm(
            await dish.get_by_id(dish_id=dish_id, submenu_id=self.submenu1.id)
        )
