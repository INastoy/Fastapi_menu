...
class TestDish:
    def setup_class(self):
        ...
        self.session.add(Menu(title='dish_test_title1',
                              description='dish_test_desc1'))
        self.session.commit()
        self.menu1: Menu = (
            self.session.query(Menu)
                .filter(
                Menu.title == 'dish_test_title1', Menu.description == 'dish_test_desc1'
            )
            .first()
        )
        ...
        self.dishes_count = (
            self.session.query(Dish).filter(
                Dish.submenu_id == self.submenu1.id).count()
        )

    def test_get_dishes_ok(self, client: TestClient, dish: DishCRUD):
        response = client.get(
            f'/api/v1/menus/{self.menu1.id}/submenus/{self.submenu1.id}/dishes'
        )
        dishes_from_db = [
            DishSchema.from_orm(dish_data)
            for dish_data in dish.get_all(submenu_id=self.submenu1.id)
        ]
    ...
    def test_get_dish_ok(self, client: TestClient, dish: DishCRUD):
        response = client.get(
            f'/api/v1/menus/{self.menu1.id}/submenus/{self.submenu1.id}/dishes/{self.dish1.id}'
        )
        dish_from_db = DishSchema.from_orm(
    ...
    def test_create_dish_ok(self, client: TestClient, dish: DishCRUD):
        ...
        dish_from_db = DishSchema.from_orm(
            dish.get_by_id(dish_id=dish_id, submenu_id=self.submenu1.id)
        )

        assert len(dish.get_all(submenu_id=self.submenu1.id)
                   ) == self.dishes_count + 1
        assert response.status_code == 201
        assert dish_from_response == dish_from_db

    def test_update_dish_ok(self, client: TestClient, dish: DishCRUD):
        ...
        dish_from_db = DishSchema.from_orm(
            dish.get_by_id(dish_id=self.dish1.id,
                           submenu_id=self.submenu1.id)
        )
    ...
    def test_delete_dish_ok(self, client: TestClient, dish: DishCRUD):
